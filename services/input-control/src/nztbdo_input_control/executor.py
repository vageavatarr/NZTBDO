...
class ActionExecutor:
    ...
    def _emit_key_action(self, action: str) -> ExecutionResult:
        ...
        if not self._send_key_windows(key):
            return ExecutionResult(action=action, performed=False, reason="sendinput_failed")
    ...
    def _send_key_windows(self, key: str) -> bool:
        ...
        sent_down = ctypes.windll.user32.SendInput(1, ctypes.byref(keydown), ctypes.sizeof(INPUT))
        sent_up = ctypes.windll.user32.SendInput(1, ctypes.byref(keyup), ctypes.sizeof(INPUT))
